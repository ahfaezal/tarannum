...
export const analyzeRecitation = async (
  ...
  try {
    ...
    if (errorMessage.toLowerCase().includes("ffmpeg not found")) {
      throw new Error("Backend ffmpeg missing for WebM conversion. Install ffmpeg or keep uploads in WAV.");
    }
    throw error instanceof Error
      ? error
      : new Error("Failed to connect to the scoring server.");
  }
};

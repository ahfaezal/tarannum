                  ...
                  <section>
                    ...
                    {flowPreview.competency_eligibility.length === 0 ? <p className="rounded-xl bg-white p-4 text-slate-600">Belum ada rakaman yang mencapai 75%.</p> : <div className="space-y-3">
                      {flowPreview.competency_eligibility.map(item => <div key={item.session_id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4">
                        <div><p className="font-bold text-slate-900">{item.reference_title}</p><p className="text-sm text-slate-600">{item.maqam || 'Maqam tidak dinyatakan'} · {new Date(item.created_at).toLocaleString('ms-MY')}</p>{item.course_title && <p className="text-xs text-slate-500">Kursus: {item.course_title}</p>}</div>
                        <div className="text-right"><p className="text-2xl font-black text-emerald-700">{Math.round(item.score)}%</p><span className={`text-xs font-bold ${item.application_status || item.can_submit ? 'text-blue-700' : 'text-amber-700'}`}>{item.application_status ? `Permohonan: ${item.application_status}` : item.can_submit ? 'Layak dihantar kepada qari' : item.blocked_reason}</span></div>
                      </div>)}
                    </div>}

...
const QariCertification: React.FC = () => {
  ...
  return <div className="mx-auto max-w-6xl space-y-6 p-4 sm:p-6 lg:p-8">
    <header><p className="text-sm font-bold uppercase tracking-wider text-emerald-700">Persijilan</p><h1 className="text-3xl font-bold text-slate-900">Semakan Sijil Kompetensi</h1><p className="mt-2 text-slate-600">Skor AI ≥75 membuka semakan. Keputusan akhir hanya dibuat selepas qari mendengar rakaman dan melengkapkan rubrik.</p></header>
    ...
    <div className="space-y-4">
      ...
      {applications.map((item) => <article key={item.id} className="rounded-2xl border bg-white p-5 shadow-sm">
        ...
        {item.status === "pending" && (() => { const draft = drafts[item.id] || emptyDraft(); const score = qariScore(draft); return <div className="mt-5 space-y-4 border-t pt-5">
          ...
          <div className="flex flex-wrap gap-2">
            <button disabled={busy === item.id || score < 75 || draft.critical_error} onClick={() => decide(item, "approved")} className="rounded-lg bg-emerald-700 px-4 py-2 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-40"><CheckCircle2 size={15} className="inline" /> Lulus & jana sijil</button>

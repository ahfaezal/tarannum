...
import { BarChart3, ClipboardCheck, LayoutDashboard, Menu, Mic2, Radio, ShieldCheck, User } from "lucide-react";
...
const AppLayout: React.FC = () => {
  ...
  const items=[{to:"/training",label:t("training"),icon:Mic2},{to:"/recording",label:t("recording"),icon:Radio},...(role==="student"?[{to:"/progress",label:t("progress"),icon:BarChart3}]:[]),...(role==="qari"?[{to:"/dashboard",label:t("dashboard"),icon:LayoutDashboard},{to:"/qari/expert-validation",label:"Penilaian Pakar",icon:ClipboardCheck}]:[]),...(role==="admin"?[{to:"/admin",label:"Administration",icon:ShieldCheck}]:[]),...(["student","qari"].includes(role||"")?[{to:"/profile",label:t("profile"),icon:User}]:[])];

...
import { Award, BarChart3, ClipboardCheck, LayoutDashboard, Menu, Mic2, Radio, ShieldCheck, User } from "lucide-react";
...
const AppLayout: React.FC = () => {
  ...
  const items=[{to:"/training",label:t("training"),icon:Mic2},{to:"/recording",label:t("recording"),icon:Radio},...(role==="student"?[{to:"/progress",label:t("progress"),icon:BarChart3},{to:"/certificates",label:"Sijil Saya",icon:Award}]:[]),...(role==="qari"?[{to:"/dashboard",label:t("dashboard"),icon:LayoutDashboard},{to:"/qari/certification",label:"Semakan Sijil",icon:Award},{to:"/qari/expert-validation",label:"Penilaian Pakar",icon:ClipboardCheck}]:[]),...(role==="admin"?[{to:"/admin",label:"Administration",icon:ShieldCheck},{to:"/admin/certification",label:"Kursus & Sijil",icon:Award}]:[]),...(["student","qari"].includes(role||"")?[{to:"/profile",label:t("profile"),icon:User}]:[])];
  const side=<><div className="flex items-center gap-3 border-b border-slate-800 p-5 font-bold"><img src="/images/logo-web.jpg" width="44" height="44" alt="Tarannum.ai logo" className="h-11 w-11 rounded-full object-cover"/><span>Tarannum.ai</span></div><nav className="flex-1 space-y-2 p-3">{items.map(({to,label,icon:Icon})=><NavLink key={to} to={to} end={to === '/admin'} onClick={()=>setOpen(false)} className={({isActive})=>`flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-semibold ${isActive?"bg-emerald-600 text-white":"text-slate-300 hover:bg-slate-800"}`}><Icon size={19}/>{label}</NavLink>)}</nav><div className="border-t border-slate-800 p-4"><div className="mb-3 truncate text-sm text-slate-300">{user?.full_name||user?.email}</div><button className="w-full rounded-lg border border-slate-700 px-3 py-2 text-sm text-slate-300" onClick={()=>{dispatch(logout());navigate("/")}}>{t("logout")}</button></div></>;

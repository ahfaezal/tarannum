...
type Enrollment = {id: string; student_id: string; student_name: string; student_email: string; attendance_status: string; valid_recording_count: number; required_recording_count: number; eligible: boolean; competency_status: string; total_recording_count: number; course_date_recording_count: number; post_course_recording_count: number; attendance_certificate_status: 'issued' | 'not_issued'; competency_certificate_status: 'issued' | 'not_issued'};
  ...
  return <section className="mb-6 rounded-2xl border border-emerald-200 bg-white p-5">
    ...
    {opened && <div className="mt-4 space-y-5">
      ...
      {course && <div className="space-y-4">
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[1280px] text-left text-sm"><thead><tr className="border-b"><th className="p-2">Peserta</th><th>Kehadiran</th><th>Latihan sah</th><th>Jumlah latihan/rakaman</th><th>Rakaman pada {new Date(course.starts_at).toLocaleDateString('ms-MY', {day:'numeric', month:'short', timeZone:'Asia/Kuala_Lumpur'})}</th><th>Rakaman selepas {new Date(course.starts_at).toLocaleDateString('ms-MY', {day:'numeric', month:'short', timeZone:'Asia/Kuala_Lumpur'})}</th><th>Kelayakan kehadiran</th>{admin && <><th>Sijil kehadiran</th><th>Sijil kompetensi</th><th>Tindakan</th></>}</tr></thead><tbody>{enrollments.map(r => <tr key={r.id} className="border-t"><td className="p-2 font-medium">{r.student_name}</td><td><select disabled={busy} value={r.attendance_status} onChange={e => {const attendance_status=e.target.value; if (!window.confirm('Sahkan perubahan kehadiran peserta ini?')) return; void run(async () => {await api(`/managed/enrollments/${r.id}/attendance`,{method:'PATCH',body:JSON.stringify({attendance_status})});await refresh();});}}><option value="registered">Belum disahkan</option><option value="attended">Hadir</option><option value="absent">Tidak hadir</option></select></td><td>{r.valid_recording_count}/{r.required_recording_count}</td><td className="font-semibold">{r.total_recording_count}</td><td>{r.course_date_recording_count}</td><td>{r.post_course_recording_count}</td><td>{r.eligible ? 'Layak' : 'Belum lengkap'}</td>{admin && <><td><span className={r.attendance_certificate_status === 'issued' ? 'font-semibold text-emerald-700' : 'text-slate-500'}>{r.attendance_certificate_status === 'issued' ? 'Dikeluarkan' : 'Belum ada'}</span></td><td><span className={r.competency_certificate_status === 'issued' ? 'font-semibold text-emerald-700' : 'text-slate-500'}>{r.competency_certificate_status === 'issued' ? 'Dikeluarkan' : 'Belum ada'}</span></td><td><button type="button" className="rounded border border-emerald-700 px-2 py-1 font-semibold text-emerald-800" onClick={() => void viewCertificates(r)}>Lihat semua sijil</button></td></>}</tr>)}</tbody></table></div>

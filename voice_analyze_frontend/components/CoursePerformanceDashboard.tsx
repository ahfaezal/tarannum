...
type Performance = {
  ...
  participants: {student_id: string; student_name: string; attendance_status: string; valid_recording_count: number; required_recording_count: number; credited_practice_minutes: number; recording_count: number; course_date_recording_count: number; post_course_recording_count: number; average_score: number | null; best_score: number | null; ai_qualified: boolean; qari_status: string}[];
  generated_at: string;
};
  ...
  return <section className="overflow-hidden rounded-2xl border border-emerald-200 bg-slate-950 text-white shadow-lg">
    ...
    <div className="grid gap-5 bg-slate-50 p-5 text-slate-900 lg:grid-cols-2 md:p-7">
      ...
      <details className="rounded-2xl border bg-white p-5 lg:col-span-2">
        ...
        <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[1050px] text-left text-sm"><thead className="bg-slate-100"><tr><th className="p-2">Peserta</th><th>Hadir</th><th>Rakaman sah (≥80%)</th><th>Jumlah rakaman</th><th>Pada {new Date(data.course.starts_at).toLocaleDateString('ms-MY', {day:'numeric', month:'short', timeZone:'Asia/Kuala_Lumpur'})}</th><th>Selepas {new Date(data.course.starts_at).toLocaleDateString('ms-MY', {day:'numeric', month:'short', timeZone:'Asia/Kuala_Lumpur'})}</th><th>Purata</th><th>Terbaik</th><th>AI ≥75</th><th>Status qari</th></tr></thead><tbody>{data.participants.map(row => <tr key={row.student_id} className="border-t"><td className="p-2 font-semibold">{row.student_name}</td><td>{row.attendance_status === 'attended' ? 'Ya' : 'Belum'}</td><td>{row.valid_recording_count}/{row.required_recording_count}</td><td className="font-semibold">{row.recording_count}</td><td>{row.course_date_recording_count}</td><td>{row.post_course_recording_count}</td><td>{score(row.average_score)}</td><td className="font-bold">{score(row.best_score)}</td><td>{row.ai_qualified ? 'Layak' : 'Belum'}</td><td>{{not_applied:'Belum hantar',pending:'Menunggu',approved:'Lulus',rejected:'Tidak lulus',resubmission_requested:'Hantar semula'}[row.qari_status] || row.qari_status}</td></tr>)}</tbody></table></div>
      </details>
